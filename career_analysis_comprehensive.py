...
#
#  응답 모델은 pydantic 으로 통일한다(규칙). v3.1 초기에는 pydantic 미설치
#  환경을 대비한 경량 대체 클래스를 병행했으나, 이는 실제 규칙에 반하는
#  임의 추가였다 — 가드/품질 테스트(hallucination_guard.py)는 애초에 이
#  응답 클래스를 쓰지 않으므로 대체 구현이 지켜주는 대상이 없었다. 제거한다.
