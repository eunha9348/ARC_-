...
- **1순위 기준은 적합도**: 채점 기준은 문항 적합성 40 + 회사·업계 부합 35 +
  직무 역량 증명 25. 지원 회사가 관심 있게 볼 경험이 먼저 선택되고,
  **무엇을 글의 중심에 둘지도 오직 이 점수가 정합니다.**
- **종합 반영**: 분량에 맞춰 핵심(깊게 서술) + 보조(짧게 연결)를 함께 뽑습니다.
  (1000자 → 핵심 1 + 보조 3, 2000자 → 핵심 2 + 보조 3 / 자동 산정)
- **시계열은 '배치 제약'일 뿐**: 시간은 선별·비중 기준이 아닙니다. 고른 경험을
  늘어놓을 때 **서술 도중 더 앞선 시점으로 되돌아가지 않게** 하는 역할만 합니다.
  적합도 1순위 경험이 2년 전 것이어도 그 경험이 글의 중심이며, 등장 순서상
  어디에 놓이든 분량의 절반 이상을 차지합니다. 두괄식 도입에서 그 경험의
  결론을 먼저 선언하는 것도 역행이 아니므로 허용됩니다.
...
- **나열 금지, 엮기는 필수**: 선정된 경험을 각각 독립 단락으로 나열하지 않고,
  적합도 1순위 경험을 6하원칙(What/When/Where/How)과 결과·성과 중심으로 깊게
  쓰되 보조 경험이 그 앞뒤를 이어 하나의 성장 곡선으로 서술
  (시계열 역행만 피하면 되며, 시간 순서가 비중을 정하지는 않음)
