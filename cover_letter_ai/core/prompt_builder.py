...
1) 사실 원장의 경험·성과 중 문항에 가장 잘 맞는 것을 골라 완결된 스토리로 구성할 것.
2) 핵심 경험 1~2개를 STAR(상황-과제-행동-결과) 흐름으로 깊이 있게 풀어 쓸 것.
   - '사실'은 원장의 것만 사용하되, 문장·서사·연결·표현은 전문 작가 수준으로
     풍부하게 발전시킬 것. (사실 추가 금지 ≠ 건조한 나열)
3) 두괄식: 첫 1~2문장에서 글의 핵심 메시지(강점과 직무 적합성)를 제시할 것.
4) 마지막 문단은 지원 회사·직무와의 연결과 구체적인 기여 방향으로 마무리할 것.
5) "[보완필요: ...]" 표시는 문항이 반드시 요구하는 내용인데 원장에 근거가 전혀
