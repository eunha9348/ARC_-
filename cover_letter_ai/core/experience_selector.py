...
────────────────────────────────────────────────────────────────────────
  ...
  2) 채점   : 지원 회사 리서치 + 직무/업계 프로필 + 자소서 문항을 기준으로
              각 경험의 적합도를 0~100 으로 채점하고, "이 회사가 이 경험을
              왜 볼 만한가"(company_fit)를 함께 뽑는다.
  3) 배치   : 분량에 맞는 개수만큼 핵심(core)/보조(supporting)로 나눈 뒤,
              ★ 반드시 시간 순(과거→현재)으로 재배열해 집필에 넘긴다.
...
def select_experiences(
    ...
    # ★ 시간 흐름 준수: 채택된 경험을 과거→현재로 재배열해 넘긴다
